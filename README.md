...
在入场菜单点击「车辆与喷漆」，可选择「远行者」运输车或「仙后座 MR-7」六轮勘探车，
并使用原厂涂装、五款预设车漆或自选颜色。拖动三维预览或点击左右箭头可旋转查看。
点击「使用这套外观」保存到当前浏览器，应用于新任务和继续任务；返回菜单或按 Esc
会放弃本次未保存的修改。两种车型共用驾驶性能，外观选择与任务存档分别保存。

外观与保存检查：`node tools/check-vehicle-appearance.mjs`。
